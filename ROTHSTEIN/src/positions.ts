...
// ─── Pass 2: Resolution Correction ──────────────────────────────────────────
// Binance resolves immediately (Pass 1), but tight-margin contracts can be wrong.
// This checks recently-resolved positions against the CLOB API's explicit `winner`
// field and corrects any mismatches. Only acts on definitive winner=true/false.

export async function checkResolutionCorrections(): Promise<void> {
  const now = Date.now();
  const CORRECTION_WINDOW_MS = 120_000; // Check positions resolved in last 2 minutes

  for (const pos of closedPositions) {
    // Skip if not recently resolved
    if (!pos.closedAt || now - pos.closedAt > CORRECTION_WINDOW_MS) continue;

    // Skip non-standard resolutions (TP exits, timeouts) — only correct Binance calls
    if (pos.status !== "RESOLVED_WIN" && pos.status !== "RESOLVED_LOSS") continue;

    // Skip if already correction-checked (use a marker on the position)
    if ((pos as any)._correctionChecked) continue;

    // Query CLOB API for official winner
    const apiResult = await fetchResolutionFromApi(pos.trade.conditionId, pos.trade.side);

    // If API hasn't settled yet (winner undefined), skip — we'll check again next cycle
    if (apiResult === null) continue;

    // Mark as checked so we don't re-query
    (pos as any)._correctionChecked = true;

    // Check if API agrees with our resolution
    const currentlyWon = pos.resolution === "WIN";
    if (apiResult === currentlyWon) continue; // Agreement — no correction needed

    // ═══ CORRECTION NEEDED ═══
    const oldStatus = pos.status;
    const oldPnl = pos.pnl || 0;
    const newWon = apiResult;
    const newStatus: PositionStatus = newWon ? "RESOLVED_WIN" : "RESOLVED_LOSS";
    const newPnl = pnl.computePnl(pos.trade.entryPrice, pos.trade.shares, newWon);

    logger.warn("positions",
      `CORRECTION: ${pos.id} ${pos.trade.asset} ${pos.trade.side} ` +
      `${oldStatus} → ${newStatus} | PnL: ${oldPnl.toFixed(4)} → ${newPnl.toFixed(4)} ` +
      `(CLOB API winner=${newWon})`
    );

    // Update position
    pos.status = newStatus;
    pos.resolution = newWon ? "WIN" : "LOSS";
    pos.exitPrice = newWon ? 1.0 : 0.0;
    pos.pnl = newPnl;

    // Adjust risk manager stats: reverse old result, record corrected result
    risk.recordCorrection(oldPnl, currentlyWon, newPnl, newWon);

    // Update decision log
    backfillResolution(pos.trade.conditionId, pos.resolution, newWon, newPnl);

    // Persist correction to JSONL
    appendJsonl(CONFIG.positionsFile, {
      type: "CORRECTED",
      ts: Date.now(),
      id: pos.id,
      ...
      oldPnl,
      newPnl,
      reason: "CLOB_API_WINNER_DISAGREED",
    });
  }
}
